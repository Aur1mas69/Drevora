...
This Policy explains how personal information is collected, used, stored and protected when people visit DREVORA websites, create or use accounts, use the web/PWA, Android or iOS applications, contact support or use DREVORA features.
...
When device permission is granted, Vehicle and Tyre Check data may also include start location, completion location, latitude, longitude, device-reported accuracy and the location capture timestamp. This is a one-time capture at start and completion, not continuous tracking.
...
- browser, mobile app/WebView or device environment information;
- screen dimensions;
- locale and timezone;
...
4.1 The Android app, iOS app and supported web/PWA environments may use secure platform storage for authentication, session information and preferences, so that authentication, preferences and supported workflows can function reliably.
...
4.4 Camera and photo-library or file access are used only when the Worker uses a feature requiring a photograph or attachment.

4.5 Biometric app lock is handled by the device operating system. DREVORA does not receive or store fingerprint templates, Face ID face images or biometric templates.

4.6 Vehicle Check location is requested only when relevant to the Vehicle Check workflow and only when device permission is granted. The app attempts a one-time location capture when a Vehicle Check starts and another one-time capture immediately before completion.
...
4.8 Denying location permission, or a failed or timed-out location lookup, does not prevent the Worker from completing the Vehicle Check.
...
Third-party service providers that receive user personal data are required to apply appropriate data-protection safeguards and to provide protection equivalent to that required by this Privacy Policy and applicable data-protection law.
...
- secure session storage on supported mobile apps using platform security where implemented, including Apple platform security and Keychain on iOS;
