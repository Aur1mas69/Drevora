...
import type { VehicleComplianceRecordFormInput } from '@/lib/complianceTypes'
import { VEHICLE_COMPLIANCE_DOCUMENT_TYPES } from '@/lib/complianceTypes'
import type { Vehicle } from '@/services/vehiclesService'
import { Upload, X } from 'lucide-react'
import { useEffect, useMemo, useState } from 'react'

type AddVehicleComplianceRecordModalProps = {
  isOpen: boolean
  vehicles: Vehicle[]
...
export function AddVehicleComplianceRecordModal({
  isOpen,
  vehicles,
  defaultVehicleId,
  isSaving = false,
  onClose,
  onSubmit,
}: AddVehicleComplianceRecordModalProps) {
  const [vehicleId, setVehicleId] = useState('')
  const [documentType, setDocumentType] = useState<string>('Inspection Certificates')
  ...
  useEffect(() => {
    ...
    setDocumentType('Inspection Certificates')
    setDocumentName('')
    setIssueDate('')
    setExpiryDate('')
    setReferenceNumber('')
    setNotes('')
    setError(null)
  }, [defaultVehicleId, isOpen, sortedVehicles])
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 px-4 py-8 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-[18px] bg-white p-5 shadow-[0_30px_80px_rgba(15,23,42,0.24)] ring-1 ring-blue-100 sm:p-6" role="dialog" aria-modal="true">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold tracking-[-0.03em] text-[#2A376F]">Add Vehicle Compliance Record</h2>
            <p className="mt-1 text-sm text-slate-500">Track inspections, certificates and other fleet documents.</p>
          </div>
        ...
        <form onSubmit={(event) => void handleSubmit(event)} className="mt-5 space-y-4">
          <label className="block text-sm font-medium text-slate-700">
            Vehicle
            <select value={vehicleId} onChange={(event) => setVehicleId(event.target.value)} className={selectClassName} required disabled={Boolean(defaultVehicleId)}>
              {sortedVehicles.map((vehicle) => (
                <option key={vehicle.id} value={vehicle.id}>{vehicle.registration} · {vehicle.make} {vehicle.model}</option>
              ))}
            </select>
          </label>
          ...
          <div className="flex flex-wrap justify-end gap-2 pt-1">
            ...
            <Button type="submit" disabled={isSaving || sortedVehicles.length === 0} className="h-10 rounded-[12px] bg-[#2563EB] px-4 text-sm font-semibold text-white hover:bg-[#1d4ed8]">
