...
type WorkerUploadCardDef = {
  key: keyof WorkerUploadSummaryStats
  filter: DocumentWorkerUploadStatusFilter
  label: string
  icon: LucideIcon
  accent: SummaryAccent
}

const WORKER_UPLOAD_CARDS: WorkerUploadCardDef[] = [
  {
    key: 'total',
    filter: 'all',
    label: 'Total Uploads',
...
type WorkerUploadSummaryCardsProps = {
  stats: WorkerUploadSummaryStats
  isLoading?: boolean
  activeFilter: DocumentWorkerUploadStatusFilter
  onSelect: (filter: DocumentWorkerUploadStatusFilter) => void
}

export function WorkerUploadSummaryCards({
  stats,
  isLoading = false,
  activeFilter,
  onSelect,
}: WorkerUploadSummaryCardsProps) {
  return (
    <div className="grid grid-cols-2 gap-2.5 sm:gap-3 md:grid-cols-3 lg:grid-cols-5">
      {WORKER_UPLOAD_CARDS.map(({ key, filter, label, icon: Icon, accent }, index) => {
        const styles = accentStyles[accent]
        const isActive = activeFilter === filter
        const isLastOddMobile = index === WORKER_UPLOAD_CARDS.length - 1
        ...
        return (
          <button
            ...
            type="button"
            aria-pressed={isActive}
            aria-label={`${label}: ${stats[key]}. ${isActive ? 'Selected filter' : 'Apply filter'}`}
            onClick={() => {
          ...
          >
            ...
            <div className="min-w-0">
              <p className="truncate text-xs font-semibold text-[#5D7C9D] dark:text-slate-400">
                {label}
              </p>
              <p className="mt-0.5 text-lg font-semibold tabular-nums tracking-[-0.02em] text-[#163A63] dark:text-slate-100 sm:text-xl">
                {isLoading ? '—' : stats[key]}
