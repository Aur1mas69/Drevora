...
import { VEHICLE_CHECK_PAGE_SIZE_OPTIONS } from '@/lib/vehicleCheckTypes'
import {
  adminHeading,
  adminSelectXs,
  adminTableFooter,
...
type VehicleChecksPaginationProps = {
  page: number
  pageSize: number
  totalCount: number
...
export function VehicleChecksPagination({
  page,
  pageSize,
  totalCount,
  onPageChange,
  onPageSizeChange,
}: VehicleChecksPaginationProps) {
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2">
        <label className={`flex items-center gap-1.5 text-xs font-medium ${adminTextMuted}`}>
          ...
          >
            {VEHICLE_CHECK_PAGE_SIZE_OPTIONS.map((size) => (
