...
import { TYRE_CHECK_PAGE_SIZE_OPTIONS } from '@/lib/tyreCheckTypes'
import { ChevronLeft, ChevronRight } from 'lucide-react'

type TyreChecksPaginationProps = {
  page: number
  pageSize: number
  totalCount: number
...
export function TyreChecksPagination({
  page,
  pageSize,
  totalCount,
  onPageChange,
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2">
        <label className={`flex items-center gap-1.5 text-xs font-medium ${adminTextMuted}`}>
          ...
          >
            {TYRE_CHECK_PAGE_SIZE_OPTIONS.map((size) => (
        ...
        <div className="flex items-center gap-1">
          ...
          <span
            className={`min-w-[4.5rem] text-center text-xs font-semibold tabular-nums ${adminHeading}`}
          >
            {page} / {totalPages}
          </span>
          <Button
