...
export function RestoreWorkerModal({
  driver,
  errorMessage,
  isRestoring,
  onCancel,
  onConfirm,
}: RestoreWorkerModalProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 px-4 py-8 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-[20px] bg-white p-5 shadow-[0_30px_80px_rgba(15,23,42,0.24)] ring-1 ring-blue-100 dark:bg-slate-950 dark:ring-white/10 sm:p-6">
        ...
        <p className="mt-3 text-sm font-medium leading-6 text-slate-500 dark:text-slate-400">
          {getDriverName(driver)} will return to the Active list and occupy one
          plan seat again. Vehicle assignments are not restored automatically.
        </p>
        {driver.archivedAt ? (
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            Archived on:{' '}
            <span className="font-semibold">{driver.archivedAt.slice(0, 10)}</span>
          </p>
        ) : null}
        {driver.retentionExpiresAt ? (
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            ...
            <span className="font-semibold">
              {driver.retentionExpiresAt.slice(0, 10)}
            </span>
          </p>
        ) : null}
