-- DREVORA legal document versions + acceptances.
...
-- -----------------------------------------------------------------------------
-- 1) Company legal controller fields
-- -----------------------------------------------------------------------------
...
-- Ensure checks/unique exist (strengthen content_hash if an older weak check exists).
do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'legal_document_versions_document_type_check'
      and conrelid = 'public.legal_document_versions'::regclass
  ) then
    alter table public.legal_document_versions
      add constraint legal_document_versions_document_type_check check (
        document_type in ('customer_terms', 'dpa', 'privacy_policy', 'worker_terms')
      );
  end if;
  ...
  if not exists (
    select 1 from pg_constraint
    where conname = 'legal_document_versions_audience_check'
      and conrelid = 'public.legal_document_versions'::regclass
  ) then
    alter table public.legal_document_versions
      add constraint legal_document_versions_audience_check check (
        audience in ('customer_admin', 'worker', 'both')
      );
  end if;

  -- Replace weak or missing content_hash check with sha256 hex.
  if exists (
    select 1 from pg_constraint
  ...
  if not exists (
    select 1 from pg_constraint
    where conname = 'legal_document_versions_type_version_unique'
      and conrelid = 'public.legal_document_versions'::regclass
  ) then
    alter table public.legal_document_versions
      add constraint legal_document_versions_type_version_unique unique (document_type, version);
  end if;
end $$;

-- Seed published versions (safe):
--   * Insert missing seed rows with is_current = true (clean install).
--   * On conflict (same type+version): refresh metadata ONLY when the row is not yet
--     published (published_at is null). Published metadata/hashes stay immutable.
--   * Never blanket-demote is_current for all rows of these types.
--   * Promote seed versions to current only when no other current version of a
--     different version string already exists for that document_type.
insert into public.legal_document_versions (
  ...
  (
    ...
    date '2026-08-01',
    '683e5e8fb1aca9bc706a3117d4bc371fc9a3911d5a8a9234e88c09a275168b8e',
    'customer_admin',
    true
  ),
  (
...
-- Promote canonical seed versions to current only if no other current version exists.
update public.legal_document_versions seed
set is_current = true
where (seed.document_type, seed.version) in (
  ('customer_terms', '0.2'),
  ('dpa', '0.2'),
  ('privacy_policy', '0.2'),
  ('worker_terms', '0.1')
...
and not exists (
  select 1
  from public.legal_document_versions other
  where other.document_type = seed.document_type
    and other.is_current = true
    and other.version is distinct from seed.version
);

-- Validate canonical seed rows match expected title/date/audience/hash.
do $$
declare
  v_mismatch text;
begin
  ...
  if v_mismatch is not null then
    raise exception 'LEGAL_SEED_MISMATCH: canonical legal_document_versions do not match expected metadata/hashes: %',
      v_mismatch;
  end if;
end $$;

-- -----------------------------------------------------------------------------
-- 3) legal_acceptances (immutable audit rows)
-- -----------------------------------------------------------------------------
...
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'legal_acceptances_document_type_check'
  ...
  if not exists (
    select 1 from pg_constraint
    where conname = 'legal_acceptances_subject_type_check'
      and conrelid = 'public.legal_acceptances'::regclass
  ) then
    alter table public.legal_acceptances
      add constraint legal_acceptances_subject_type_check check (
        subject_type in ('customer_admin', 'worker')
      );
  end if;
  ...
  if not exists (
    select 1 from pg_constraint
    where conname = 'legal_acceptances_acceptance_action_check'
      and conrelid = 'public.legal_acceptances'::regclass
  ) then
    alter table public.legal_acceptances
      add constraint legal_acceptances_acceptance_action_check check (
        acceptance_action in ('accepted', 'acknowledged')
      );
  end if;
  ...
  if not exists (
    select 1 from pg_constraint
    where conname = 'legal_acceptances_acceptance_source_check'
      and conrelid = 'public.legal_acceptances'::regclass
  ) then
    alter table public.legal_acceptances
      add constraint legal_acceptances_acceptance_source_check check (
        acceptance_source in (
          'onboarding',
          'trial',
          'subscription',
  ...
  if not exists (
    select 1 from pg_constraint
    where conname = 'legal_acceptances_platform_check'
      and conrelid = 'public.legal_acceptances'::regclass
  ) then
    alter table public.legal_acceptances
      add constraint legal_acceptances_platform_check check (
        platform in ('android', 'web', 'pwa')
      );
  end if;
  ...
  -- Strengthen subject_refs (drop weak then add).
  if exists (
    select 1 from pg_constraint
  ...
  if not exists (
    select 1 from pg_constraint
    where conname = 'legal_acceptances_doc_subject_check'
      and conrelid = 'public.legal_acceptances'::regclass
  ) then
    alter table public.legal_acceptances
      add constraint legal_acceptances_doc_subject_check check (
        (
          document_type in ('customer_terms', 'dpa')
          and subject_type = 'customer_admin'
        )
...
begin
  ...
  -- Allow is_current toggles only; published metadata/hashes are immutable.
...
-- Resolve acceptor display name server-side. Never trust client-supplied names.
-- Order: auth.users meta full_name/name → linked driver first/last → email local-part.
...
declare
  ...
  v_docs jsonb;
begin
  -- Keep deprecated param referenced so callers may still send it; value is ignored.
  ...
  -- ACCEPT requires Admin (not broader office roles).
...
begin
  ...
  -- Status remains office-role (TM/Office can see incomplete state).
